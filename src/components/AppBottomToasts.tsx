...
function StaleDataToast() {
  ...
  const minutes = Math.round(STALE_DATA_AFTER_MS / 60_000);

  return (
    <div
      className="flex w-full items-center gap-inline rounded-2xl border border-border-subtle bg-surface-elevated px-4 py-3 shadow-xl shadow-black/40"
      role="status"
    >
      ...
      <span className="min-w-0 flex-1 text-body-sm text-muted">
        Your data may be outdated (not refreshed in {minutes}+ minutes). Pull fresh data?
      </span>
      <button
        type="button"
